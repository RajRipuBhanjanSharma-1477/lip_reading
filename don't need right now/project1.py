...
import threading
from component.mylibary import video
...
def audio_plot():
    ...
    # graph plot
    data=np.frombuffer(stream.read(512,exception_on_overflow=False),dtype="int16")
    x1=blank_image.shape[1]//8
    y1=blank_image.shape[0]//2
    x2=blank_image.shape[1]//8
    # print(data)
...
def toggle():
    ...
    if not camera_running:
        ...
        video1=threading.Thread(target=video)
        video1.start()
        p=pyaudio.PyAudio()
        # audio graph
        stream=p.open(
        ...
        audio1=threading.Thread(target=audio_plot)
        audio1.start()
    else:
